/**
 * NHL '93 Game Mode Runtime
 * Embedded from the homepage's Open Lane experience.
 * ---------------------
...
const canvasFrame = document.querySelector('.rink-canvas-frame');
...
const gameMenus = document.querySelectorAll('.game-menu');
...
const MAX_CANVAS_PIXELS = 2560 * 1440; // cap the backing buffer for stable full-screen performance
...
const GOAL_ZONES = [
  {
    id: 'north',
    rect: { x: 1700, y: 875, width: 600, height: 120 },
    scorer: 'player',
  },
  {
    id: 'south',
    rect: { x: 1790, y: 5100, width: 540, height: 120 },
    scorer: 'opponent',
  },
];
...
function fitCameraToCanvas() {
  if (!canvas || !game.camera || !game.world || canvas.height <= 0) return;
  const aspectRatio = canvas.width / canvas.height;
  if (!Number.isFinite(aspectRatio) || aspectRatio <= 0) return;

  const baseAspectRatio = CAMERA_VIEW.width / CAMERA_VIEW.height;
  let viewWidth;
  let viewHeight;

  if (aspectRatio >= baseAspectRatio) {
    viewHeight = CAMERA_VIEW.height;
    viewWidth = viewHeight * aspectRatio;
  } else {
    viewWidth = CAMERA_VIEW.width;
    viewHeight = viewWidth / aspectRatio;
  }

  if (viewWidth > game.world.width) {
    viewWidth = game.world.width;
    viewHeight = viewWidth / aspectRatio;
  }
  if (viewHeight > game.world.height) {
    viewHeight = game.world.height;
    viewWidth = viewHeight * aspectRatio;
  }

  game.camera.width = Math.min(viewWidth, game.world.width);
  game.camera.height = Math.min(viewHeight, game.world.height);
  clampCamera(game.camera, game.world);
}

function resizeGameCanvas() {
  if (!canvas) return;
  const target = canvasFrame || canvas;
  ...
  if (requestedPixels > MAX_CANVAS_PIXELS) {
    renderScale = Math.sqrt(MAX_CANVAS_PIXELS / (cssWidth * cssHeight));
  }
...
function setupResponsiveCanvas() {
  ...
  if (typeof ResizeObserver === 'function') {
    game.canvasResizeObserver = new ResizeObserver(resizeGameCanvas);
    game.canvasResizeObserver.observe(canvasFrame);
  } else {
  ...
  if (puckPath) {
    try {
      ...
      const needsSingleFrame =
        puckImage.width < variant.frameSize.width ||
        puckImage.height < variant.frameSize.height;
      const puckConfig = needsSingleFrame
        ? createSingleFramePuckConfig(variant, puckImage)
        : variant;
...
function reportMissingNpcVariants(npcConfig, spriteVariants) {
  ...
  if (missing.length > 0) {
    console.warn(
      'NPC config references sprite variants that do not exist:',
      missing
    );
  }
  return missing;
...
async function toggleFullscreen() {
  if (!gameStage || !fullscreenToggle) return;
  try {
    ...
    flashStatus('Fullscreen is unavailable in this browser.', 1800);
  }
}

if (fullscreenToggle) {
  const fullscreenSupported =
    Boolean(gameStage?.requestFullscreen) && Boolean(document.exitFullscreen);
  ...
  try {
    ...
    if (game.npcs && game.npcs.length) {
      game.npcs.forEach((npc) => {
        const npcVariant =
          variant.id === npc.variantId
            ? variant
            : game.spriteVariantsById?.[npc.variantId];
...
async function loadNpcSkaters(npcConfig, variantIndex, world, bounds) {
  ...
  for (const npcDef of npcConfig.players) {
    ...
    if (!variant) {
      console.warn(
        `NPC "${npcDef.id}" references missing sprite variant "${npcDef.spriteVariant}".`
      );
      continue;
    }
    try {
...
function updateSkaterState(skater, inputVector, physics = PLAYER_PHYSICS) {
  ...
  const facingAngle =
    speed > physics.MIN_SPEED
      ? Math.atan2(skater.velocity.y, skater.velocity.x)
      : skater.heading;
  const headingDelta = Math.abs(
    normalizeAngle(skater.targetHeading - skater.heading)
  );
...
function updateSkaterAnimation(skater, deltaMs) {
  ...
  if (skater.animation.elapsed >= frameDuration) {
    ...
    skater.animation.frameIndex =
      (skater.animation.frameIndex + 1) % dir.frames.length;
...
function updateStaticNpc(npc, deltaSeconds, deltaMs) {
  ...
  if (speed > NPC_HIT_MIN_VELOCITY) {
    ...
    npc.state = 'skating';
    npc.direction = quantizeDirection(
      Math.atan2(npc.velocity.y, npc.velocity.x)
    );
  } else {
    npc.velocity.x = 0;
    npc.velocity.y = 0;
...
function applyNpcHitFromPlayer(npc, player) {
  ...
  const impulse =
    Math.min(speed, PLAYER_PHYSICS.MAX_SPEED) * NPC_HIT_IMPULSE_SCALE;
  if (impulse <= 0) return;
...
function createPuck(puckConfig, puckImage, world) {
  ...
  const frameSize = puckConfig.frameSize || {
    width: puckImage.width,
    height: puckImage.height,
  };
  const origin = puckConfig.origin || {
    x: frameSize.width / 2,
...
function resetPuckToCenter() {
  ...
  const center = game.puckConfig?.spawn || {
...
function cloneNpc(template, duplicateIndex = 0) {
  ...
  npc.position.x =
    template.position.x +
    duplicateIndex * 30 * (duplicateIndex % 2 === 0 ? 1 : -1);
...
class HotspotHud {
  ...
  update(activeHotspot, hints, { canActivate = true } = {}) {
    ...
    if (activeHotspot) {
      ...
      if (this.actionEl) {
        const actionMessage = canActivate
          ? 'Press Enter to open'
          : 'Grab the puck to activate';
...
function handleHotspotActivation() {
  ...
  if (statusEl) {
    statusEl.textContent = formatSystemLabel(
      `Opening ${game.activeHotspot.label}`
    );
    statusEl.removeAttribute('hidden');
  }
...
function handlePuckShot() {
  ...
  const heading =
    speedVectorLength > PLAYER_PHYSICS.MIN_SPEED
...
function render() {
  ...
  const sortedSkaters = [...game.skaters].sort(
  ...
  hotspotHud.update(game.activeHotspot, game.hotspotHints, {
    canActivate: canActivateHotspot,
  });
}
...
function update(deltaMs) {
  ...
  game.skaters.forEach((skater) => {
    ...
    const controlVector = skater.controlSource
      ? skater.controlSource()
      : ZERO_VECTOR;
...
async function bootstrap() {
  ...
  try {
    const [spritesConfig, hotspotsConfig, puckConfig, npcConfig] =
      await Promise.all([
    ...
    const { base: spriteSheet, puck: puckSpriteSheet } =
      await getVariantSpriteSheets(initialVariant);
    const world = {
